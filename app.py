...
def process_images_with_progress(image_paths, model, task_id):
    ...
    # Function to process a single image and update progress
    def process_single_image(idx_and_path):
        ...
        try:
            ...
            # Update task status
            task['current_page'] = idx + 1
            task['message'] = f"Extracting text from page {idx+1} of {task['total_pages']}..."
            ...
            if extracted_text is None or "--- ERROR:" in extracted_text:
                results[idx] = extracted_text or f"--- ERROR EXTRACTING PAGE {idx+1} ---"
                success = False
            else:
                results[idx] = extracted_text
                success = True
            ...
            task['percentage'] = base_percentage_after_conversion + int((processed_count / task['total_pages']) * extraction_percentage_span)
            processing_tasks[task_id] = task  # Update task
            
            return success
        except Exception as e:
            results[idx] = f"--- ERROR: Exception during extraction for page {idx+1}: {str(e)} ---"
            return False
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(process_single_image, indexed_paths)
...
def process_pdf_with_progress(task_id, filepath, api_key, output_format):
    ...
    try:
        ...
        task['processing_time'] = f"{processing_time:.2f} seconds" # Add processing time info
        
        if has_errors_during_extraction:
            task['message'] = f'Processing complete with some errors in {processing_time:.2f} seconds. Please check the output file.'
        else:
            task['message'] = f'Processing complete in {processing_time:.2f} seconds! Your file is ready for download.'
